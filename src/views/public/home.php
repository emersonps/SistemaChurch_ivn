                    ...
                    <div class="flat-strip-track">
                        ...
                        <?php foreach ($eventos as $evento): ?>
                            ...
                            <div class="flat-strip-card">
                                ...
                                <?php foreach ($eventoContacts as $ec): ?>
                                    <a href="https://wa.me/<?= htmlspecialchars($ec['wa_digits']) ?>" target="_blank" rel="noopener noreferrer" class="flat-strip-card-contact">
                                        <i class="fab fa-whatsapp"></i>
                                        <span><?= htmlspecialchars($ec['role'] !== '' ? $ec['role'] : 'WhatsApp') ?>: <?= htmlspecialchars($ec['phone']) ?></span>
                                    </a>
                                <?php endforeach; ?>
                        ...
                        <?php foreach ($convites as $convite): ?>
                            ...
                            <div class="flat-strip-card convite-card">
                                ...
                                <div class="convite-card-body">
                                    ...
                                    <?php endif; ?>
                                    <?php foreach ($conviteContacts as $cc0): ?>
                                        <a href="https://wa.me/<?= htmlspecialchars($cc0['wa_digits']) ?>" target="_blank" rel="noopener noreferrer" class="convite-card-contact">
                                            <i class="fab fa-whatsapp"></i>
                                            <span><?= htmlspecialchars($cc0['role'] !== '' ? $cc0['role'] : 'WhatsApp') ?>: <?= htmlspecialchars($cc0['phone']) ?></span>
                                        </a>
                                    <?php endforeach; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            sectionCarousels.forEach(function(carousel) {
                ...
                const scope = carousel.closest('[data-carousel-group]') || carousel;
                ...
                function updateUI() {
                    ...
                    const idx = getIndex();
                    prevBtns.forEach(function(btn) { btn.disabled = idx <= 0; });
                    nextBtns.forEach(function(btn) { btn.disabled = total ? idx >= total - 1 : true; });
                ...
                prevBtns.forEach(function(button) {
                    button.addEventListener('click', function() { goTo('prev'); });
                });
                nextBtns.forEach(function(button) {
                    button.addEventListener('click', function() { goTo('next'); });
                });
                dots.forEach(function(dot) {
                    dot.addEventListener('click', function() {
                        const index = parseInt(dot.getAttribute('data-section-dot'), 10) || 0;
